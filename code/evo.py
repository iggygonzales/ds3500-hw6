...
class Environment:
    ...
    def run_agent(self, name):
        """ Invoke an agent against the population (changed a little) """
        op, k = self.agents[name]
        # if requires 1 solution (mutate)
        if k == 1:
            pick = self.get_random_solutions(1)[0]
            new_solution = op(pick)
        # if requires 2 (crossover)
        elif k == 2:
            pick1, pick2 = self.get_random_solutions(2)
            new_solution = op(pick1, pick2)
